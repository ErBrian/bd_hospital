...
CREATE TABLE hospitales ( cod_hospital INTEGER, nombre VARCHAR(50), direccion VARCHAR(50), num_plazas INTEGER, primary key(cod_hospital) );

CREATE TABLE personas ( cod_hospital INTEGER, dni INTEGER PRIMARY KEY, apellidos VARCHAR(50), funcion VARCHAR(30), salario INTEGER,

FOREIGN KEY (cod_hospital) REFERENCES hospitales(cod_hospital)

);

CREATE TABLE medicos ( cod_hospital INTEGER, dni INTEGER PRIMARY KEY, apellidos VARCHAR(50), especialidad VARCHAR(50),
